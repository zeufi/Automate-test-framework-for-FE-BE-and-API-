...
#     # new_path = webdriver_path + ':' + current_path  # For mac or linux
...
def create_output_directory(prefix='results_'):
    ...
    curr_file_path = pathlib.Path(__file__).parent.absolute()  # Gives the path of runner.py
...
if __name__ == '__main__':
    ...
    command = f'behave -k --no-capture -f allure_behave.formatter:AllureFormatter -o "{allure_out_dir}" ' \
              f'-f json.pretty -o "{json_out_dir}" ' \
              f'--junit --junit-directory "{junit_out_dir}" ' \
              f'"{behave_options}" "{test_dir}" '
